...
def to_complex(data):
    assert data.size(-1) == 2
    data_real, data_imag = torch.unbind(data, -1)
    return torch.complex(data_real, data_imag)


def to_channel(data):
    # data is complex
    return torch.stack([data.real, data.imag], -1)



def fft2n(data, normalized='ortho'):
    """
    Apply centered 2 dimensional Fast Fourier Transform.
    ...
    assert data.size(-1) == 2
    data = ifftshift(data, dim=(-3, -2))
    data = torch.fft.fft2(to_complex(data), norm=normalized)
    data = fftshift(to_channel(data), dim=(-3, -2))

    return data


def rfft2n(data, normalized='ortho'):
    """
    Apply centered 2 dimensional Fast Fourier Transform.
    ...
    """
    data = ifftshift(data, dim=(-2, -1))
    data = torch.fft.rfft2(data, norm=normalized)
    data = fftshift(to_channel(data), dim=(-3, -2))
    return data


def ifft2n(data, normalized='ortho'):
    """
    Apply centered 2-dimensional Inverse Fast Fourier Transform.
    ...
    assert data.size(-1) == 2
    data = ifftshift(data, dim=(-3, -2))
    data = torch.fft.ifft2(to_complex(data), norm=normalized)
    data = fftshift(to_channel(data), dim=(-3, -2))
    return data


def irfft2n(data, normalized='ortho'):
    """
    Apply centered 2-dimensional Inverse Fast Fourier Transform.
    ...
    """
    data = ifftshift(data, dim=(-3, -2))
    data = torch.fft.irfft2(to_complex(data), norm=normalized)
    data = fftshift(data, dim=(-2, -1))
    return data
...
def get_hf_lf(data, mask_wide=60):
    # mag = lambda x: (x[:, :, :, 0:1] ** 2 + x[:, :, :, 1:2] ** 2) ** 0.5
    assert data.size(-1) == 2 or data.size(1) == 2
    if data.size(1) == 2:
        data = data.permute(0, 2, 3, 1)

    # data = ifftshift(data, dim=(-3, -2))
    fft_data = to_channel(torch.fft.fft2(to_complex(data), norm='ortho'))

    cw, ch = data.size(1)//2, data.size(2)//2
    mask = torch.zeros_like(data)
    mask[:, cw - mask_wide:cw + mask_wide, ch-mask_wide:ch+mask_wide, :] = 1

    hf = fft_data * mask
    fft_data = fftshift(fft_data, dim=(-3, -2))
    lf = fft_data * mask
    return lf, hf
