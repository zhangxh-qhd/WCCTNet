...
def calculate_psnr_single(img1, img2, border=0):
    ...
    # img2 = img2.astype(np.float64)
    # gt recon
    return skimage.metrics.peak_signal_noise_ratio(img1, img2, data_range=img1.max())

def calculate_nrmse_single(img1, img2, border=0):
    # img1 =  np.clip(img1.squeeze(), -1, 1)
    # img2 = np.clip(img2.squeeze(), -1, 1)
    if not img1.shape == img2.shape:
        raise ValueError('Input images must have the same dimensions.')
    ...
    # img2 = img2.astype(np.float64)
    # gt recon
    return skimage.metrics.normalized_root_mse(img1, img2, normalization="min-max")
...
def calculate_ssim_single(img1, img2, border=0):
    ...
    return skimage.metrics.structural_similarity(img1, img2, data_range=img1.max())
